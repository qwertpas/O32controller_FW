...
void foc_startup() {
    ...
    // 90º out of phase would be 1/4th of an electrical cycle, so 1170.285 angle
    // counts
    m_angle = (uint16_t)((p.spi_RX[0] << 8) + p.spi_RX[1] + 16384); // 0 to 32767
    e_offset = (m_angle * PPAIRS - e_offset) &
               (32768 - 1); // convert to electrical angle, modulo 32768
    e_angle = 0;
...
void foc_loop() {
    ...
    if (p.print_flag) { // 100Hz clock
        ...
        rpm = ((cont_angle - cont_angle_prev) * 100 * 60) >>
              15; // should be accurate within reasonable RPM range if 32-bit
        ...
        // p.uart_RX[0], p.uart_RX[1], p.uart_RX[2]); 		sprintf((char*)
        // p.uart_TX, " freq: %d\n I_d_filt: %d\n I_q_filt: %d\n \t", loop_freq,
        // I_d_filt, I_q_filt); 		sprintf((char*) p.uart_TX, " I_u: %d \n
        // I_v: %d \n I_w: %d \n I_d: %d \n I_q: %d \n \t", I_u, I_v, I_w, I_d,
        // I_q); sprintf((char*) p.uart_TX, " rpm: %ld\n m_angle: %d\n e_angle: %d\n
        // revs: %ld\n cont_angle: %ld\n \t", rpm, m_angle, e_angle, revs,
        // cont_angle);
